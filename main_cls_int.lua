...
local function shortcut(ninp, nout, direction, stride)
  if stride == 1 then
    if ninp == nout then
      return nn.Identity()
    else
      return nn.Sequential()
         :add(nn.Convolution(ninp, nout, 1, 1))
...
local function block_conv(ninp, nout, stride)
  local s = nn.Sequential()
  s:add(nn.SpatialConvolution(ninp,nout,3,3,stride,stride,1,1))
  s:add(nn.SpatialBatchNormalization(nout))
  s:add(nn.LeakyReLU(0.2, true))
  s:add(nn.SpatialConvolution(nout,nout,3,3,1,1,1,1))
  s:add(nn.SpatialBatchNormalization(nout))

  return nn.Sequential()
     :add(nn.ConcatTable()
        :add(s)
        :add(shortcut(DOWN, stride)))
     :add(nn.CAddTable(true))
     :add(nn.LeakyReLU(0.2, true))
end

local function block_deconv(ninp, nout, stride)
  local s = nn.Sequential()
  if stride == 1 then
    s:add(nn.SpatialConvolution(ninp,nout,3,3,stride,stride,1,1))
  elseif stride == 2 then
    s:add(nn.SpatialFullConvolution(ninp,nout,4,4,stride,stride,1,1))
  end
  s:add(nn.SpatialBatchNormalization(nout))
  s:add(nn.ReLU(true))
  s:add(nn.SpatialConvolution(nout,nout,3,3,1,1,1,1))
